...
impl RangeProof {
    ...
    pub fn from_bytes(slice: &[u8]) -> Result<RangeProof, RangeProofError> {
        ...
        let t_x = TryInto::<Option<Scalar>>::try_into(Scalar::from_canonical_bytes(util::read32(
            &slice[4 * 32..],
        )))?
        .ok_or(ProofVerificationError::Deserialization)?;

        let t_x_blinding = TryInto::<Option<Scalar>>::try_into(Scalar::from_canonical_bytes(
            util::read32(&slice[5 * 32..]),
        ))?
        .ok_or(ProofVerificationError::Deserialization)?;

        let e_blinding = TryInto::<Option<Scalar>>::try_into(Scalar::from_canonical_bytes(
            util::read32(&slice[6 * 32..]),
        ))?
        .ok_or(ProofVerificationError::Deserialization)?;
